...
(function(global) {
    ...
    function play_WebAudioAPI(midiContent) {
        ...
        require_script(libtimidity_url, function() {
            pasameElMidiCampeon(midiContent);
        });
    }



    function pasameElMidiCampeon(midiContent) {
        midiFileArray = new Int8Array(midiContent);
    ...
    try {
        if ((bv.platform == "iPhone" || bv.platform == "iPod" || bv.platform == "iPad") && bv.majorVersion <= 6) {
            audioMethod = 'none';
        } else {
    ...
    global.MIDIjs.message_callback = function(message) {
        console.log(message);
    };
